 ...
 * @business() for processing/business logic. Then create an object of your subclass, supply a @View object and start the
 * process by calling @normForm.
...
abstract class AbstractNormForm {
    ...
    /** @var View $currentView Holds the currently supplied @View object that will be used in the template to render output. */
    protected $currentView;

    /** @var array $errorMessages An array containing all error messages being set by @isValid(). */
    protected $errorMessages;

    /** @var string $statusMessage An optional status message that can be set in @business() when processing data was successful. */
    ...
    /**
     * Creates a new instance for a norm form object and initializes all necessary fields. A @View object is used to initially
     * define how and where output is displayed via the template engine and supply parameters to the template. The template
     * engine itself is also set up, two optional parameters allow setting the template paths.
    ...
    /**
     * Main "decision" method for the form processing. This decision tree uses @isFormSubmission() to check if the form is
     * being initially displayed or shown again after a form submission and either calls @show() to display the form (using
     * the supplied @View object) or validate the received input in @isValid(). If validation failed, @show() is called again.
     * Possible error messages provided as parameters to the @View object in @isValid() can now be displayed. Once the submission
     * was correct, @business() is called where the data can be processed as needed. If this method returns a new @View object
     * it is used to display the output as specified there, otherwise the current view is kept.
     */
     ...
     * template (types FORM or TEMPLATE) then the parameters supplied with the @View object are passed on to the template
     * engine and the template file specified in the @View object is displayed. If the output is in another, external PHP
     * file (type URL), the parameters supplied in the @View object are converted to a valid query parameter string which
     * is then appended to the defined file name and a HTTP redirect to the assembled URL is initiated.
     * @param View|null $view An optional @View object used to display output or null to use the initially supplied view.
     */
